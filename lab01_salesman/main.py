...
for dimmension in range(5, max_cities + 1):
    ...
    for graph in graphs:
        ...
        with open("results.md", "a") as file:
            file.write(f"| {dimmension} | {graph[1]} | {graph[2]} | Brute force DFS | {bf.get('dfs')[0]} | {bf.get('dfs')[1]} |\n")
            file.write(f"| {dimmension} | {graph[1]} | {graph[2]} | Brute force BFS | {bf.get('bfs')[0]} | {bf.get('bfs')[1]} |\n")
            file.write(f"| {dimmension} | {graph[1]} | {graph[2]} | Greedy search | {gs[0]} | {gs[1]} |\n")
            file.write(f"| {dimmension} | {graph[1]} | {graph[2]} | A* AVG | {hs.get('avg')[0]} | {hs.get('avg')[1]} |\n")
            file.write(f"| {dimmension} | {graph[1]} | {graph[2]} | A* MIN | {hs.get('min')[0]} | {hs.get('min')[1]} |\n")
